...
# 3. 最後才是 mischief_js 字串
mischief_js = """
<div id="container" style="height: 500px; width: 100%; position: relative; border-radius: 20px; overflow: hidden; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding-top: 40px; background-color: #ffffff; border: 1px solid #eee;">
    ...
    <button id="catchMe" style="position: absolute; top: 75%; left: 50%; transform: translate(-50%, -50%); padding: 12px 35px; background-color: #FF4B4B; color: white; border: none; border-radius: 50px; cursor: pointer; font-size: 18px; font-weight: bold; z-index: 10; transition: left 0.15s ease-out, top 0.15s ease-out;">確定</button>
</div>
...
<script>
    ...
    function moveButton() {
        var padding = 50;
        var btnW = btn.offsetWidth || 100;
        var btnH = btn.offsetHeight || 45;
        var newX = Math.random() * (container.clientWidth - btnW - padding * 2) + padding;
        var newY = Math.random() * (container.clientHeight - btnH - 250) + 200;
